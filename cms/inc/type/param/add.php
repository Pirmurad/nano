...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $lastorder   =   select("MAX(ordering)","`type_param`","`ordering`","DESC")->fetch(PDO::FETCH_ASSOC);
    $order      =   $lastorder['MAX(ordering)']+1;

    $data = "`name` = ?,`linkname` = ?,`type_id` = ?,`ordering` = ?,`status` = ?";
    ...
    $inserttype = insert("`type_param`", $data, $sql);

    if($inserttype) {
        redirect("type/".get('type_id')."/param/list/");
    }
    else {
        $errors['errors'] = 'Problem baş verdi, parametr əlavə olunmadı !';
        $_SESSION['errors'] = $errors;
        redirect("type/".get('type_id')."/param/create/");
    }

}else{
    $type = prepare("`name`","`type`","`id`=?",[get('type_id')])->fetch(PDO::FETCH_ASSOC);
?>
<div id="wrapper">
    ...
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
<h1 class="page-header"><strong><?= ucfirst($type['name']) ?></strong> tipinə parametr əlavə et</h1>
<div class="row">
